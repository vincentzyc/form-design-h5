<template>
  <div class="wg-phone" :style="item.style">
    <div class="wg-item" :class="[item.labelPosition==='top'?'flex-column':'align-middle']">
      <div class="wg-title flex-none" v-show="item.showLabel" :style="{width:item.labelwidth/50+'rem'}">{{item.labelTitle}}</div>
      <div class="flex-auto">
        <input class="wg-input" type="tel" maxlength="11" v-model="item.value" :placeholder="item.placeholder">
      </div>
    </div>
    <div class="wg-item" v-if="item.showCode" :class="[item.labelPosition==='top'?'flex-column':'align-middle']">
      <div class="wg-title flex-none" v-show="item.showLabel" :style="{width:item.labelwidth/50+'rem'}">验证码</div>
